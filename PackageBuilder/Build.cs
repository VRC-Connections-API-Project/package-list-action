...
namespace VRC.PackageManagement.Automation
{
    ...
    partial class Build : NukeBuild
    {
        ...
        [Parameter("Authorization Header for not GitHub")]
        ...
        Target BuildRepoListing => _ => _
            .Executes(async () =>
            {
                ...
                foreach (string url in possibleReleaseUrls)
                {
                    ...
                    if (currentPackageUrls.Contains(MapToPublishZipUrl(url)))
                    {
                    ...
                    manifest.url = MapToPublishZipUrl(url);
                ...
                await Task.WhenAll(zipFiles.Select(SaveZipFile));
        ...
        {
            var response = await GetAuthenticatedResponse(url);
            if (response.IsSuccessStatusCode)
            {
                var bytes = await response.Content.ReadAsByteArrayAsync();
                var savePath = DownloadPathForUrl(url);
                Directory.CreateDirectory(savePath.Parent);
                await File.WriteAllBytesAsync(savePath, bytes);
            }
            else
            {
                Serilog.Log.Error($"Could not download zip file from {url}");
            }
        }
        ...
        async Task<HttpResponseMessage> GetAuthenticatedResponse(string url)
        {
                   ...
                   new HttpRequestMessage(HttpMethod.Get, url))
            {
                ...
                if (IsServerBuild)
                {
                    var isGitHub = url.StartsWith("https://github.com/") || url.StartsWith("https://api.github.com/");
                    if (isGitHub)
                    {
                    ...
                    else if (AuthorizationHeader != null)
                    {
                        ...
                        requestMessage.Headers.Authorization =
                            new AuthenticationHeaderValue(header[0], header[1]);
                    }
                }
